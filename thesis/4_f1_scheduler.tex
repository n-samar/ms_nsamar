...
% dsm: Commented out for revision
...
%the standard method (\autoref{sec:fhe_analysis}) uses keyswitch hints of size $2L^2$ polynomials,
%and the alternative method uses keyswitch hints of size $2(L + 73)$,
...
%The goal of this phase is to  schedule that minimizes
%data movement while achieving good parallelism.
...
% axelf: I know this is confusing... thoughts about how to improve it?
...
% dsm: I think this is a bit of a no-op at best. At worst, it can be misinterpreted as "we overdesigned on ports because the scheduler is bad". I think we can safely omit it.
% axelf: from masters thesis
%Our highly banked scratchpad and many-ported register files substantially simplify on-chip scheduling.
%Each resource constraint has the possibility of stalling an instruction; a fact that is mitigated by our large number of ports.
