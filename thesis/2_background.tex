...
We now describe how encrypted data (ciphertexts) is represented and processed
in FHE schemes. The implementation of each computation on ciphertext data is
called a \emph{homomorphic operation}. For example, the \emph{homomorphic
multiplication} of two ciphertexts yields another ciphertext that, when
decrypted, is the element-wise multiplication of the encrypted plaintexts. For
concreteness, we introduce FHE using the CKKS scheme, and briefly discuss other
FHE schemes in \autoref{sec:fhe_others}.
...
CKKS uses two noise management techniques in tandem: \emph{rescaling} and
\emph{bootstrapping}.
...
budget of a ciphertext. Bootstrapping enables computations of arbitrary
depth by separating them into regions of limited depth.
