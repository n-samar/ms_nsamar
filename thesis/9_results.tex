...
%Note that FUs are pipelined, and some FUs are not fully utilized internally (e.g., $L<64$ leave some CRB pipelines idle).
...
% dsm: Uncommented this, I hadn't noticed it was commented. This is literally EDP... it's a widely used metric because perfomance generally costs energy. That EDP >> Perf improvmeent is noteworthy. Is it just the product of both? Sure. Doesn't make it meaningless though.
% alex: I don't think it makes sense to report this number --- it's just
%       <gmean_speadup> * <gmean_energy improvement> = 12.3 x 18.7`. Both
%       numbers are about 10x because we do 10x better bootstapping.
%       Multiplying them is double counting.
%       Dividing them gives a meaningful number---F1+ would consume
%       18.7/12.3 = 1.5x more power than CL.
...
and \name has
% dsm: Not true, it has 1/4th the lanes but we have more FUs/lane
%half as many lane groups as F1+ has compute clusters.
...
The benchmarks presented so far meet an 80-bit security level,
...
% axelf: Discussion of SP vs. FP
% We modify all benchmarks to use \emph{densely packed} bootstrapping.
% Implementation choices in most existing state-of-the-art FHE libraries force application programmers to split up their data across \emph{multiple} ciphertexts to avoid the performance penalty of bootstrapping ciphertexts with many slots.
% For example, the base ResNet-20 implementation stores data in 64 separate ciphertexts each only encoding 64 values instead of a single ciphertext encoding 4096 values.
% This algorithmic choice results in over 96\% of execution time being spent bootstrapping. By moving to deeper, more fully packed bootstrapping algorithms, we are able to reduce bootstrapping to 42\% of our CPU execution time and speed up our CPU implementation from 6.25 hours to 23 minutes (a $16 \times$ speedup).
% Importantly, denser packing also benefits \name. For ResNet-20, denser packing allows us to reduce execution time from 2.3s to 264ms, and for logistic regression, it reduces execution time from 468ms to 121ms, resulting in $8.7 \times$ and $3.9 \times$ speedups respectively.
